...
Future generatePdf(
    ...
    String table8Title,
    String table8Description,
    List<String> years,
    List<double> totalTreesPlanted,
    List<double> totalCarbonAbsorbed,
    List<double> carbonCredits,
    List<double> parisGoals) async {
  ...
  pdf.addPage(
    pw.MultiPage(
      ...
      build: (pw.Context context) => [
        ...
        pw.Table.fromTextArray(
          border: pw.TableBorder.all(),
          headers: ['Tree Species', 'Total Trees', 'Carbon Absorbed (kg)'],
          data: List.generate(
              treeSpecies.length,
              (index) => [
                    treeSpecies[index] ?? '',
        ...
        pw.Container(
          ...
          child: pw.Chart(
            grid: pw.CartesianGrid(
              xAxis: pw.FixedAxis(
                List.generate(treeSpecies.length, (index) => index.toDouble()),
                marginStart: 10,
                marginEnd: 10,
                divisions: true,
              ),
        ...
        pw.Table.fromTextArray(
          ...
          data: List.generate(
            ...
            (index) => [
              years[index] ?? '',
              totalTreesPlanted.isNotEmpty && index < totalTreesPlanted.length
                  ? totalTreesPlanted[index].toString()
                  : '',
              totalCarbonAbsorbed.isNotEmpty &&
                      index < totalCarbonAbsorbed.length
                  ? totalCarbonAbsorbed[index].toString()
                  : '',
              carbonCredits.isNotEmpty && index < carbonCredits.length
                  ? carbonCredits[index].toString()
                  : '',
              parisGoals.isNotEmpty && index < parisGoals.length
                  ? '${parisGoals[index]}%'
                  : '',
            ],
          ),
